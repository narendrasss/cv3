import { omit } from 'lodash'
import * as types from './types'
import { types as transactionTypes, AddTransactionAction, DeleteTransactionAction } from '../transactions'
import { BudgetState } from './interfaces'
import { AppAction } from '../interfaces'

/*
state shape:
{
  [name]: {
    name: string
    budget: number
    spent: number
    transactions: number[]
    color: string
  }
}
*/

const initialState = {} as BudgetState

const addTransactionToBudget = (state: BudgetState, action: AddTransactionAction) => {
  const { id, budget, amount, type } = action.payload
  if (type === 'income') return state
  const prevBudget = state[budget]
  return {
    ...state,
    [budget]: {
      ...prevBudget,
      spent: Number(prevBudget.spent) + Number(amount),
      transactions: [...prevBudget.transactions, id]
    }
  }
}
const deleteTransactionFromBudget = (state: BudgetState, action: DeleteTransactionAction) => {
  const { id, budget, amount, type } = action.payload
  if (type === 'income') return state
  const prevBudget = state[budget]
  return {
    ...state,
    [budget]: {
      ...prevBudget,
      spent: Number(prevBudget.spent) - Number(amount),
      transactions: prevBudget.transactions.filter(bid => bid !== id)
    }
  }
}

const reducer = (state = initialState, action: AppAction) => {
  switch (action.type) {
    case types.ADD:
      return {
        ...state,
        [action.payload.name]: {
          ...action.payload
        }
      }
    case types.EDIT:
      return {
        ...state,
        [action.payload.name]: {
          ...state[action.payload.name],
          ...action.payload
        }
      }
    case types.DELETE:
      return omit(state, action.payload)
    case transactionTypes.ADD:
      return addTransactionToBudget(state, action)
    case transactionTypes.DELETE:
      return deleteTransactionFromBudget(state, action)
    default:
      return state
  }
}

export default reducer
